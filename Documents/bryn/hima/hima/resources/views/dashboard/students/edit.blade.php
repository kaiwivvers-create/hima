...
<section class="card" style="max-width: 700px;">
    <form method="POST" action="{{ route('dashboard.students.update', ['student' => $student, 'lang' => app()->getLocale()]) }}">
        ...
        <div class="field">
            ...
            <select id="parent_user_id" name="parent_user_id" class="js-parent-select" data-name-target="parent_name_preview" data-contact-target="parent_contact_preview" required>
                ...
                @foreach ($parents as $parent)
                    <option value="{{ $parent->id }}" data-parent-name="{{ $parent->name }}" data-parent-contact="{{ $parent->email }}" @selected((string) old('parent_user_id', $student->parent_user_id) === (string) $parent->id)>
        ...
        <div class="field">
            ...
            <input id="parent_name_preview" type="text" value="{{ old('parent_name_preview', $student->parent_name) }}" readonly>
        </div>

        <div class="field">
            <label for="parent_contact_preview">Parent Contact (auto)</label>
            <input id="parent_contact_preview" type="text" value="{{ old('parent_contact_preview', $student->parent_contact) }}" readonly>
        </div>
