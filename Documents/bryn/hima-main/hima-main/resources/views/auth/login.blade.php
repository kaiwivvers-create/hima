...
<body>
    <header class="topbar page-reveal delay-1">
        <div class="actions">
            ...
            <a class="top-link" href="{{ route('register') }}">
    ...
    <section class="hero page-reveal delay-2">
        <div class="hero-content">
            <h1>{{ __('auth.login_hero_title') }}</h1>
            <p>{{ __('auth.login_hero_text') }}</p>
        </div>
    </section>

    <div class="container">
        <section class="auth-wrap">
            <form method="POST" action="{{ route('login.post') }}" class="card page-reveal delay-3">
                @csrf
                ...
                <button class="btn" type="submit">
                    <span class="btn-content">
                        <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M10 17v-3h7v-4h-7V7l-5 5 5 5zm3 5H6a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h7v2H6v16h7v2z"/></svg>
                        {{ __('auth.login_title') }}
                    </span>
                </button>
                <p class="muted">{{ __('auth.no_account') }} <a href="{{ route('register') }}">{{ __('auth.top_register') }}</a></p>
            </form>
        </section>
    </div>
