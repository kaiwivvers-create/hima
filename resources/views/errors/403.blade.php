...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .shape { position:absolute; right:-50px; bottom:-50px; width:160px; height:160px; background:radial-gradient(circle at 30% 30%, #fff2b3, #ffd86a); border-radius:40%; opacity:.6; }
    </style>
</head>
<body>
    <div class="card">
        ...
        <h1>{{ __('errors.403_heading') }}</h1>
        <p class="muted">{{ __('errors.403_detail') }}</p>
        @if (!empty($exception?->getMessage()))
            <p class="muted">{{ __('errors.error_reason') }} {{ $exception->getMessage() }}</p>
        @endif
        <p class="muted" style="margin-top:.4rem; font-size:.95rem;">{{ __('errors.403_hint') }}</p>
        <div class="actions">
